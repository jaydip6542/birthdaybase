import 'package:active_ecommerce_flutter/repositories/businessseting_repository.dart';
...
class _CartState extends State<Cart> {
 ...
 int minmumcartvalue = 2000;
  ...
  onQuantityIncrease(seller_index, item_index) {
    if (_shopList[seller_index].cart_items[item_index].quantity < _shopList[seller_index].cart_items[item_index].upper_limit) {

      up= _shopList[seller_index].cart_items[item_index].quantity;
      _shopList[seller_index].cart_items[item_index].quantity=up+_shopList[seller_index].cart_items[item_index].lower_limit ;
      getSetCartTotal();
      setState(() {
  ...
  onQuantityDecrease(seller_index, item_index) {
    if (_shopList[seller_index].cart_items[item_index].quantity > _shopList[seller_index].cart_items[item_index].lower_limit) {
      up= _shopList[seller_index].cart_items[item_index].quantity;
  ...
  onPressProceedToShipping() async {
    var businesssiteResponse = await BusinessSeting().getbusinessseting();
    print(businesssiteResponse.data.length);
    if (businesssiteResponse.success == false) {


      ToastComponent.showDialog("something went wrong", context,
          gravity: Toast.CENTER, duration: Toast.LENGTH_LONG);
    } else {
        print(businesssiteResponse.data[121].value);
        print(_cartTotal);
        var one = int.parse(businesssiteResponse.data[121].value);
        if(one >_cartTotal ){

          ToastComponent.showDialog("Minimum order value ${businesssiteResponse.data[121].value  }", context,
              gravity: Toast.CENTER, duration: Toast.LENGTH_LONG);
                    ...
                    child: FlatButton(
                      ...
                      onPressed: () async {
                        ...
                        for(int i=0;i<_shopList.length;i++) {
                          ...
                          print(partialTotal(i));

                          if(partialTotal(i) >= 2000){
                            flag = 1;
                          ...
                          if (businesssiteResponse.success == false) {
                            ...
                            if(one >_cartTotal ){
                              ...
                              process(mode: "proceed_to_shipping");
                                                 ...
                                                 // showDialog(
                           //     context: context,
                           //     builder: (BuildContext context) =>_showDialogIMPS(context));
                        }
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        //   padding: const EdgeInsets.symmetric(horizontal: 8.0),
                        //   child: Text(
                        //     partialTotal(index) >= 2000?"":"Minimum value 2000",
                        //     style: TextStyle(
                        //         color: MyTheme.accent_color,
                        //         fontSize: 16,
  ...
  Widget _showDialogIMPS(BuildContext context) {

    return Dialog(
        child: Container(
          // margin: EdgeInsets.only(left: wp(2),right: wp(2)),
          height: 100,
          ...
          child: Container(
              ...
              child: Center(
              child: Text(
                'Minimum Order Value Per Seller  is ${"Rs."}2000',
